...
export function TrialDocumentCreateNotice({ className }: TrialDocumentCreateNoticeProps) {
  ...
  if (loading || !usage || usage.subscriptionStatus !== "trialing") {
    return null;
  }
      ...
      <div className="flex gap-2">
        ...
        <div className="space-y-2">
          <p className="font-medium text-foreground">
            {exhausted ? t("trialLimitBannerTitle") : t("trialRemainingBanner", { remaining })}
          </p>
          {exhausted ? (
            <>
              <p className="text-muted-foreground">{t("trialLimitBannerHint")}</p>
