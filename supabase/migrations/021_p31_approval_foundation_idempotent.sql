-- P3.1 retry / idempotent completion
-- Use this if 020 deadlocked or partially applied.
-- Run once in Supabase SQL Editor during low traffic (pause app if possible).
...
-- ─── 3. Document columns (no inline FK — shorter lock) ─────────────────────
...
-- FK constraints added separately (NOT VALID → VALIDATE = less blocking)
DO $$
DECLARE
  rec RECORD;
BEGIN
  ...
  LOOP
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint WHERE conname = rec.cname
    ) THEN
      EXECUTE format(
...
-- Backfill: only rows that need correction (small vs full table scan)
...
BEGIN
  ...
  END IF;
  SELECT company_id, status INTO v_company_id, v_status FROM receipt_vouchers WHERE id = p_id;
  ...
  IF v_status = 'cancelled' THEN RAISE EXCEPTION 'Receipt voucher already cancelled' USING ERRCODE = '23505'; END IF;
  UPDATE receipt_vouchers SET status = 'cancelled', lifecycle_status = 'cancelled',
    cancelled_at = NOW(), cancel_reason = trim(p_reason), cancelled_by = auth.uid()
...
BEGIN
  ...
  IF COALESCE(length(trim(p_reason)), 0) < 3 THEN
    RAISE EXCEPTION 'Validation error: cancel reason is required' USING ERRCODE = '22023';
  END IF;
  SELECT company_id, status INTO v_company_id, v_status FROM payment_vouchers WHERE id = p_id;
  IF v_company_id IS NULL THEN RAISE EXCEPTION 'Payment voucher not found' USING ERRCODE = 'P0001'; END IF;
  IF NOT public.user_has_company_role(v_company_id, 'accountant') THEN RAISE EXCEPTION 'Forbidden' USING ERRCODE = '42501'; END IF;
  IF v_status = 'cancelled' THEN RAISE EXCEPTION 'Payment voucher already cancelled' USING ERRCODE = '23505'; END IF;
  UPDATE payment_vouchers SET status = 'cancelled', lifecycle_status = 'cancelled',
    cancelled_at = NOW(), cancel_reason = trim(p_reason), cancelled_by = auth.uid()
  WHERE id = p_id AND status = 'active';
...
BEGIN
  ...
  IF COALESCE(length(trim(p_reason)), 0) < 3 THEN
    RAISE EXCEPTION 'Validation error: cancel reason is required' USING ERRCODE = '22023';
  END IF;
  SELECT company_id, status INTO v_company_id, v_status FROM invoices WHERE id = p_id;
  IF v_company_id IS NULL THEN RAISE EXCEPTION 'Invoice not found' USING ERRCODE = 'P0001'; END IF;
  IF NOT public.user_has_company_role(v_company_id, 'accountant') THEN RAISE EXCEPTION 'Forbidden' USING ERRCODE = '42501'; END IF;
  IF v_status = 'cancelled' THEN RAISE EXCEPTION 'Invoice already cancelled' USING ERRCODE = '23505'; END IF;
  UPDATE invoices SET status = 'cancelled', lifecycle_status = 'cancelled',
    cancelled_at = NOW(), cancel_reason = trim(p_reason), cancelled_by = auth.uid()
  WHERE id = p_id AND status = 'active';
