-- P3.3: Receipt voucher customer approval flow (receipts only)

-- ─── 1. Draft receipts without official numbers ─────────────────────────────

ALTER TABLE receipt_vouchers ALTER COLUMN number DROP NOT NULL;
ALTER TABLE receipt_vouchers ALTER COLUMN display_number DROP NOT NULL;

ALTER TABLE receipt_vouchers DROP CONSTRAINT IF EXISTS receipt_vouchers_company_id_number_key;

CREATE UNIQUE INDEX IF NOT EXISTS idx_receipt_vouchers_issued_company_number
...
BEGIN
  ...
  IF v_lifecycle <> 'draft' THEN
    RAISE EXCEPTION 'Receipt must be in draft status to send for approval' USING ERRCODE = '22023';
  END IF;
...
CREATE OR REPLACE FUNCTION public.get_receipt_approval_by_hash(p_token_hash TEXT)
RETURNS TABLE (
  receipt_id UUID,
  company_id UUID,
  company_name TEXT,
...
BEGIN
  ...
  SELECT * INTO v_receipt FROM receipt_vouchers WHERE id = v_token.document_id;
  ...
  WHERE document_type = 'receipt_voucher'
    AND document_id = v_receipt.id
    AND version = v_receipt.approval_snapshot_version;
  ...
  customer_name := COALESCE(v_customer.name, v_receipt.party_name);
  ...
  lifecycle_status := v_receipt.lifecycle_status;
  ...
  token_valid := v_token.used_at IS NULL
    ...
    AND v_receipt.lifecycle_status = 'pending_approval';

  RETURN NEXT;
END;
$$;

-- ─── 4. Approve receipt by token (public) ───────────────────────────────────
...
-- ─── 5. Reject receipt by token (public) ────────────────────────────────────
...
-- ─── 6. Allow cancelling draft / pending / rejected receipts ────────────────
...
GRANT EXECUTE ON FUNCTION public.send_receipt_for_approval(UUID, TEXT, TIMESTAMPTZ, JSONB) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_receipt_approval_by_hash(TEXT) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.approve_receipt_by_hash(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.reject_receipt_by_hash(TEXT, TEXT, TEXT, TEXT) TO anon, authenticated, service_role;
